...
export function initRecentlyViewed(root = document) {
  ...
  if (!currentScript || !grid) return;

  const { i18n, ...current } = JSON.parse(currentScript.textContent);
  ...
  const others = existing.filter((item) => item.handle !== current.handle).slice(0, MAX_SHOWN);
